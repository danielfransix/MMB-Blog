...
export default function HeroCarousel({ posts }: HeroCarouselProps) {
  ...
  // Use all passed posts
  const displayPosts = posts;
  const maxIndex = displayPosts.length - 1;

  // Calculate card width
  // Desktop: 35% of viewport (half of previous 70%)
  // Mobile: 85% of viewport for better readability
  const isMobile = viewportWidth < 768;
  const cardWidth = viewportWidth * (isMobile ? 0.85 : 0.35);
  const gap = 16;
